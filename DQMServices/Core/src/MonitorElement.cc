...
static TH1* checkRootObject(const std::string& name, TObject* tobj,
                            const char* func, int reqdim)
{
  if (!tobj)
...
MonitorElement::MonitorElement(const MonitorElement& x,
    ...
    : data_(x.data_), scalar_(x.scalar_), object_(nullptr),
      reference_(x.reference_), refvalue_(nullptr), qreports_(x.qreports_)
{
}
...
{
  update();
  if (kind() == DQM_KIND_TH1F || kind() == DQM_KIND_TH1S ||
      kind() == DQM_KIND_TH1D) {
    int nbins = getNbinsX();
...
/// returns value of ME in string format (eg. "f = 3.14151926" for double
/// numbers);
...
std::string MonitorElement::qualityTagString(const DQMNet::QValue& qv) const
{
  ...
  result.reserve(7 + 2 * titlelen + buflen + qv.algorithm.size() +
                 qv.message.size());
  result += '<';
  result += data_.objname;
  result += '.';
...
MonitorElement::runQTests()
{
  ...
  for (size_t i = 0, e = data_.qreports.size(); i < e; ++i) {
    ...
    // if (qc && (dirty || qc->wasModified()))  // removed for new QTest
    // (abm-090503)
    if (qc && dirty) {
...
/// get RMS uncertainty of histogram along x, y or z axis(axis=1,2,3
/// respectively)
...
void MonitorElement::setBinLabel(int bin, const std::string& label,
                                 int axis /* = 1 */)
{
  update();
...
/// set the time offset, if option = "gmt" then the offset is treated as a GMT
/// time
...
TAxis* MonitorElement::getAxis(const char* func, int axis) const
{
  ...
  if (!a)
    raiseDQMError("MonitorElement", "No such axis %d in monitor element"
                                    " '%s' of type '%s'",
                  axis, data_.objname.c_str(), typeid(*h).name());
...
void MonitorElement::addProfiles(TProfile* h1, TProfile* h2, TProfile* sum,
                                 float c1, float c2)
{
  assert(h1);
  assert(h2);
  ...
  for (Int_t bin = 0, nbin = sum->GetNbinsX() + 1; bin <= nbin; ++bin) {
    Double_t entries =
        c1 * h1->GetBinEntries(bin) + c2 * h2->GetBinEntries(bin);
...
// implementation: Giuseppe.Della-Ricca@ts.infn.it
// Can be called with sum = h1 or sum = h2
void MonitorElement::addProfiles(TProfile2D* h1, TProfile2D* h2,
...
void MonitorElement::copyFunctions(TH1* from, TH1* to)
{
  ...
  for (int i = 0, nfuncs = fromf ? fromf->GetSize() : 0; i < nfuncs; ++i) {
    ...
    //  ; // FIXME? tof->Add(new TPaveStats(*stats));
      ...
      raiseDQMError(
          "MonitorElement", "Cannot extract function '%s' of type"
...
void MonitorElement::copyFrom(TH1* from)
{
  ...
  if (isSoftResetEnabled()) {
    if (kind() == DQM_KIND_TH1F || kind() == DQM_KIND_TH1S ||
        kind() == DQM_KIND_TH1D || kind() == DQM_KIND_TH2F ||
        kind() == DQM_KIND_TH2S || kind() == DQM_KIND_TH2D ||
        kind() == DQM_KIND_TH3F)
      // subtract "reference"
...
void MonitorElement::getQReport(bool create, const std::string& qtname,
                                QReport*& qr, DQMNet::QValue*& qv)
{
  assert(qreports_.size() == data_.qreports.size());
