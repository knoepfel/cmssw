...
public:
  ...
  static constexpr uint32_t DQM_PROP_REPORT_ALARM =
      (DQM_PROP_REPORT_ERROR | DQM_PROP_REPORT_WARN | DQM_PROP_REPORT_OTHER);
  ...
  struct HashOp {
    uint32_t operator()(Object const& a) const { return a.hash; }
  };

  struct HashEqual {
    bool operator()(Object const& a, Object const& b) const
    {
      return a.hash == b.hash && *a.dirname == *b.dirname &&
             a.objname == b.objname;
    }
  };
...
#define dqmhashfinal(a, b, c)                                                  \
  {                                                                            \
    ...
    auto k = (unsigned char const*)key;
...
#undef dqmhashfinal
  ...
  static void unpackQualityData(QReports& qr, uint32_t& flags,
                                char const* from);
...
protected:
  ...
  void waitForData(Peer* p, std::string const& name, std::string const& info,
                   Peer* owner);
  virtual void releaseFromWait(Bucket* msg, WaitObject& w, Object* o);
  virtual bool onMessage(Bucket* msg, Peer* p, unsigned char* data, size_t len);

  virtual Object* findObject(Peer* p, std::string const& name,
                             Peer** owner = nullptr) = 0;
  virtual Object* makeObject(Peer* p, std::string const& name) = 0;
...
private:
  void losePeer(char const* reason, Peer* peer, lat::IOSelectEvent* event,
                lat::Error* err = nullptr);
  void requestObjectData(Peer* p, char const* name, size_t len);
  void releaseFromWait(WaitList::iterator i, Object* o);
  void releaseWaiters(std::string const& name, Object* o);
...
template <class ObjType> class DQMImplNet : public DQMNet {
public:
  struct ImplPeer;
  ...
  DQMImplNet(std::string const& appname = "") : DQMNet(appname) {}

protected:
  Object* findObject(Peer* p, std::string const& name,
                     Peer** owner = nullptr) override
  {
    ...
    if (p) {
      ...
      for (i = peers_.begin(), e = peers_.end(); i != e; ++i) {
        auto pos = i->second.objs.find(proto);
  ...
  {
    uint64_t minreq =
        (lat::Time::current() - lat::TimeSpan(0, 0, 5 /* minutes */, 0, 0))
            .ns();
    auto* ip = static_cast<ImplPeer*>(p);
      ...
      for (oi = pi->second.objs.begin(), oe = pi->second.objs.end(); oi != oe;
        ...
        if (all || (oi->flags & DQM_PROP_NEW))
          size += 9 * sizeof(uint32_t) + oi->dirname->size() +
                  oi->objname.size() + 1 + oi->scalar.size() +
